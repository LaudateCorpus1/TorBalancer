...
do
	...
	if [ ! -d "data/polipo/polipo$i" ]; then
		echo "Creating directory data/polipo/polipo$i"
		mkdir "data/polipo/polipo$i"
	fi
	...
	echo "Running: polipo daemonise=true pidFile=data/polipo/polipo$i/pid proxyPort=$http_port socksParentProxy=localhost:$socks_port logFile=data/polipo/polipo$i/polipo_$http_port.log diskCacheRoot='' disableLocalInterface=true"
	polipo daemonise=true pidFile=data/polipo/polipo$i/pid proxyPort=$http_port socksParentProxy=localhost:$socks_port logFile=data/polipo/polipo$i/polipo_$http_port.log diskCacheRoot='' disableLocalInterface=true
done

haproxy -f rotating-tor-proxies.cfg

echo "Polipo processes:"
ps aux | grep polipo | grep daemonise | wc -l
echo "Tor processes:"
